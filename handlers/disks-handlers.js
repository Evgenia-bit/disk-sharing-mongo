...
module.exports = {
    ...
    giveAwayDisk: async (req, res) => {
        try {
            ...
            if(!diskId) return res.json({status: 'error', msg: 'Данные не верны'})
            const takenItem = await db.getTakenItem(diskId)
            if(!takenItem) return res.json({status: 'error', msg: 'Такого диска не существует'})
            if(takenItem.ownerid == req.session.person_id) return res.json({status: 'error', msg: 'Это ваш собственный диск!'})
            if(takenItem.holderid != req.session.person_id) return res.json({status: 'error', msg: 'Вы не брали этот диск!'})
    ...
    takeDisk: async (req, res) => {
        try {
            ...
            if(!takenItem) return res.json({status: 'error', msg: 'Такого диска не существует'})
            if(takenItem.ownerid == req.session.person_id) return res.json({status: 'error', msg: 'Это ваш собственный диск!'})
            if(takenItem.holderid == req.session.person_id) return res.json({status: 'error', msg: 'Вы уже взяли этот диск!'})
            if(takenItem.ownerid != takenItem.holderid) return res.json({status: 'error', msg: 'Этот диск уже взят другим пользователем!'})
